...
function OscillatorInner({ lesson }: { lesson: ExperimentLessonData }) {
  const { isPlaying, ensureStarted, start, stop } = useAudioLab();
  const analyser = useAnalyserNode();
  const chainRef = useRef<ReturnType<typeof createOscillatorChain> | null>(
    null
  );
...
export function OscillatorPlayground({
  lesson,
}: {
